    ...
    public class LocaleEN : IDictionarySource
    {
        ...
        public IEnumerable<KeyValuePair<string, string>> ReadEntries(IList<IDictionaryEntryError> errors, Dictionary<string, int> indexCounts)
        {
            ...
            return new Dictionary<string, string>
            {
                ...
                { m_Setting.GetOptionGroupLocaleID(Setting.PrefabModifier), Setting.PrefabModifier },

                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.Disclaimer)), "DISCLAIMER: This mod modifies prefabs in an irreversible way. Please ensure you have a backup of the StreamingData~ before using it, as changes cannot be undone. The mod author is not responsible for any loss of content resulting from its use.\r\nA game restart is absolutely necessary for the modified assets to be shown up in game.\r\nIf any prefabs from mods_subscribed is modified using any of the methods below, it will change the file integrity of the Prefab thus will show up as \"dirty/modified\" is Simple Mod Checker's mod verification. To restore, simply delete the folder and let the game (or Skyve) redownload it for you." },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.Disclaimer)), "" },

                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.CreatePackagePath)), "Path to folders to create packages with" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.CreatePackagePath)), $"Every subfolder in this folder will have one '.cok' file each.\r\n{PathInfo}" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.CreatePackage)), "Create Binary Package(s)" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.CreatePackage)), $"Create '.cok' packages to 'StreamingData~\\~CreatedPackages' from each subfolders or prefab files.\r\n- If the folder has subfolder, each subfolder will be used to create one cok file, any separate files in the folder will only be reset..\r\n- If the folder only contains files, no folders, then one cok will be created for everything inside the folder.\r\nThe packages won't be available in-game by default, since that will conflict with the original prefab files.\r\nRemove/disable the original prefab folder and move the '.cok' files out of the '~CreatedPackages' folder (or remove the ~ in front of '~CreatedPackages')." },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.CreatePackageAny)), "Create Package as is" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.CreatePackageAny)), $"Create '.cok' package to 'StreamingData~\\~CreatedPackages'\r\n- This will create one package for the whole selected path. No prefab processing will be done, but supports subfolders.\r\nThe packages won't be available in-game by default, since that will conflict with the original prefab files.\r\nRemove/disable the original prefab folder and move the '.cok' files out of the '~CreatedPackages' folder (or remove the ~ in front of '~CreatedPackages')." },
